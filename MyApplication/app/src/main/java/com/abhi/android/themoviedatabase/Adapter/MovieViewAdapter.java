...
public class MovieViewAdapter extends RecyclerView.Adapter<MovieViewAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(final MyViewHolder holder, final int position) {
        ...
        if(PopularMovies.imageSize == 1 || TopRatedMovies.imageSize == 1 || NewMovies.imageSize == 1 || FavoriteMovies.imageSize == 1){
            Picasso.with(mContext).load(movie.getMovie_poster_path()).resize(200,450).into(holder.thumbnail);
        }else if(PopularMovies.imageSize == 1 || TopRatedMovies.imageSize == 1 || NewMovies.imageSize == 1 || FavoriteMovies.imageSize == 1){
            Picasso.with(mContext).load(movie.getMovie_poster_path()).resize(200,400).into(holder.thumbnail);
        }else{
            Picasso.with(mContext).load(movie.getMovie_poster_path()).resize(200,340).into(holder.thumbnail);
        }
